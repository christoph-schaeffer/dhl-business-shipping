 ...
 */
class doManifest extends AbstractResponse {
     ...
     */
    public function __construct(Request\doManifest $request, $soapResponse, $soapRequest, $languageLocale) {
        parent::__construct($request, $soapResponse, $soapRequest, $languageLocale);

        if(!property_exists($soapResponse, 'ManifestState'))
            return;

        if(is_array($soapResponse->ManifestState)):
            $this->ManifestStates = $this->mapMultipleManifestStates($soapResponse->ManifestState, $languageLocale);
        elseif(!empty($soapResponse->ManifestState)):
            $this->ManifestStates[] = $this->mapManifestState($soapResponse->ManifestState, $languageLocale);
        endif;
    }
    ...
    private function mapManifestState($manifestStateResponse, $languageLocale) {
        ...
        if(property_exists($manifestStateResponse, 'Status'))
            $manifestState->Status = StatusMapper::getStatusObjects($manifestStateResponse->Status, $languageLocale);
