 ...
 */
class getSignatureTest extends AbstractUnitTest {

    public function testConstructWithCode100WithoutErrorResponse() {
        $request      = new Request\Tracking\getSignature();
        $restResponse =  simplexml_load_string('<?xml version="1.0" encoding="UTF-8"?><data code="100" request-id="1337"></data>');

        $responseDe = new Response\Tracking\getSignature($request, $restResponse, 'rawRequestTest', 'de');
        ...
        $responseEn = new Response\Tracking\getSignature($request, $restResponse, 'rawRequestTest', 'en');
